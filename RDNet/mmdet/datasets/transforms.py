...
class ImageTransform(object):
    ...
    def __call__(self, img, scale, flip=False, keep_ratio=True):
        '''resize'''
        if keep_ratio:  # aspect
            # √
            # print(img.shape)  # ori
            # mmcv.imrescale - Resize image while keeping the aspect ratio
        ...
        if flip:
            img = mmcv.imflip(img)
        '''pad'''
        # 这里的目的是pad后保证图像可以被size_divisor整除
        if self.size_divisor is not None:
            img = mmcv.impad_to_multiple(img, self.size_divisor)
            pad_shape = img.shape
        else:
            pad_shape = img_shape
        '''transpose to RGB format'''
        if self.rgb_format:
...
class BboxTransform(object):
    ...
    def __call__(self, bboxes, img_shape, scale_factor, flip=False):
        ...
        # 将transformed bboxes限定在(0,img_shape-1)
